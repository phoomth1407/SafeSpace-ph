...
function assertOutputLanguage(ai: any, language: "th" | "en") {
  ...
  const textFields = [
    ai?.depression_chance,
    ai?.ai_summary,
    ai?.similar_case,
    ...(Array.isArray(ai?.recommendations) ? ai.recommendations : []),
    ...(Array.isArray(ai?.tool_recommendations) ? ai.tool_recommendations.map((item: any) => item?.reason) : []),
  ];
  if (textFields.some(hasThaiText)) {
    throw new Error("AI returned text in the wrong language");
  }
}
  ...
  if (apiKey) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        assertOutputLanguage(ai, language);
        aiSucceeded = true;
        result = {
