      ...
      <div className="rounded-2xl p-6 border bg-slate-900/60 border-slate-800">
        <div className="text-center">
          ...
          {isPhq9 ? (
            ...
            <><h1 className={`text-2xl font-bold ${risk.color}`}>{risk.label}</h1><div className="mt-4 mx-auto max-w-xs"><div className="h-3 bg-slate-800 rounded-full overflow-hidden"><div className={`h-full bg-gradient-to-r ${risk.bar} rounded-full transition-all duration-700`} style={{ width: `${score}%` }} /></div><div className="text-xs text-slate-400 mt-1.5">{t("result.riskScore")}: {score}/100</div></div></>
          )}
        </div>
      </div>
