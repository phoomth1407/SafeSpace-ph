...
export default function AssessmentResult() {
  ...
  useEffect(() => {
    const fetchResult = async () => {
      if (!id && location.state?.result) { setResult(location.state.result); setLoading(false); return; }
      if (!id) { setError(t("result.notfound")); setLoading(false); return; }
      try { setResult(await base44.entities.Assessment.get(id)); } catch { setError(t("result.notfound")); }
  ...
  const score = isPhq9 ? phqScore : (result.risk_score || 0);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="rounded-2xl p-6 border bg-slate-900/60 border-slate-800">
        <div className="text-center">
          <div className="text-xs text-slate-400 mb-1">{isPhq9 ? "PHQ-9 screening result" : t("result.yourResult")}</div>
          {isPhq9 ? (
            <>
      ...
      <div className="bg-amber-500/10 rounded-2xl p-4 border border-amber-500/20"><p className="text-xs text-amber-200 leading-relaxed">{lang === "en" ? "Important: This result is only an initial screening estimate based on your answers. It is not a medical diagnosis and cannot confirm any mental health condition. If your concerns continue or affect daily life, consider talking with a parent, teacher, trusted adult, or qualified mental health professional for appropriate guidance." : "หมายเหตุสำคัญ: ผลลัพธ์นี้เป็นเพียงการประเมินเบื้องต้นจากคำตอบของคุณ ไม่ใช่การวินิจฉัยทางการแพทย์ และไม่สามารถยืนยันว่าคุณมีภาวะทางสุขภาพจิตใด ๆ ได้ หากคุณรู้สึกว่าปัญหายังคงเกิดขึ้นหรือส่งผลต่อชีวิตประจำวัน ควรพูดคุยกับผู้ปกครอง ครู ผู้ใหญ่ที่ไว้ใจได้ หรือผู้เชี่ยวชาญด้านสุขภาพจิตเพื่อรับคำแนะนำที่เหมาะสม"}</p></div>
      ...
      {isGuest && <div className="bg-sky-500/10 rounded-2xl p-5 border border-sky-500/20 text-center"><p className="text-sm text-sky-200 mb-3"><AlertTriangle className="w-4 h-4 inline mr-1" />{t("result.guest.prompt")}</p><div className="flex gap-2 justify-center"><Link to="/register" className="bg-slate-100 text-slate-900 text-sm font-semibold px-4 py-2 rounded-full">{t("result.guest.register")}</Link><Link to="/login" className="bg-slate-900 text-slate-200 text-sm font-semibold px-4 py-2 rounded-full border border-slate-700">{t("result.guest.login")}</Link></div></div>}
      <div className="flex items-center gap-3"><Link to="/assessment" className="flex-1 flex items-center justify-center gap-1.5 bg-slate-100 text-slate-900 text-sm font-semibold px-4 py-2.5 rounded-full"><RotateCcw className="w-4 h-4" />{t("result.retake")}</Link><Link to="/resources" className="flex-1 flex items-center justify-center gap-1.5 bg-slate-900 text-slate-200 text-sm font-semibold px-4 py-2.5 rounded-full border border-slate-700"><Phone className="w-4 h-4" />{t("result.hotlines")}</Link></div>
    </div>
  );
}
